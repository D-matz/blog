...
/**
 * Return type-based highlighting tokens for a previously-compiled module
 * as a `{ tokens, customs }` JS object:
 *
 * - `tokens` is a flat `Uint32Array` laid out as `[start, end, marker, ...]`.
 *   `start`/`end` are byte offsets into the module's source.
 *   `marker` is `0..=10` for built-in kinds (Int, Float, String, Bool, Nil,
 *   List, Result, BitArray, Tuple, Function, Unknown — in that order) and
 *   `11 + i` for user-defined types, where `i` indexes into `customs`.
 * - `customs` is an array of `"module:name"` strings, deduplicated, in the
 *   order the markers reference them.
 *
 * The compiler makes no palette decisions: identical-identity tokens share
 * a marker, distinct-identity tokens get distinct markers. JS chooses how
 * (or whether) to alias them to colours.
 *
 * You need to call `compile_package` before calling this function, and the
 * module must have been part of the just-compiled set (cached modules
 * don't carry their typed AST in this in-memory pipeline).
 */
export function module_tokens(project_id: number, module_name: string): any;
...
export interface InitOutput {
  ...
  readonly module_tokens: (a: number, b: number, c: number) => any;
