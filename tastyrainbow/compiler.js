...
class Project {
  ...
  // Type-based highlighting tokens for the most-recently compiled module.
  //
  // Returns `{ tokens, customs }` or null if the modified compiler isn't
  // loaded (older wasm). Each token is `{start, end, marker}` where:
  //   marker 0..10 = builtin kind (Int, Float, String, Bool, Nil, List,
  //     Result, BitArray, Tuple, Function, Unknown — in that order)
  //   marker >= 11 = customs[marker - 11], a `"module:name"` identity.
  moduleTokens(moduleName) {
    if (typeof compiler.wasm.module_tokens !== "function") return null;
    const result = compiler.wasm.module_tokens(this.#id, moduleName);
    if (!result) return null;
    const flat = result.tokens;
    const tokens = new Array(flat.length / 3);
    for (let i = 0, j = 0; i < flat.length; i += 3, j += 1) {
      tokens[j] = { start: flat[i], end: flat[i + 1], marker: flat[i + 2] };
    }
    return { tokens, customs: result.customs || [] };
  }
}
