...
async function compileEval(code) {
  ...
  try {
    ...
    // Type-based highlighting tokens come from the JS compile (modules
    // aren't kept around between targets).
    const tokenResult = project.moduleTokens("main");
    if (tokenResult) {
      result.tokens = tokenResult.tokens;
      result.customs = tokenResult.customs;
    }
